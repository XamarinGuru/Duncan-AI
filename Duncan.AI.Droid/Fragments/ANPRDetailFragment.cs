...
{
	public class ANPRDetailFragment : Fragment
	{
        TextView _txtNoteId;
		...
		public override View OnCreateView (LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
		{
			...
			View view = inflater.Inflate (Resource.Layout.ANPR_detail, null);
            _prefs = PreferenceManager.GetDefaultSharedPreferences(Activity);

            _txtNoteId = view.FindViewById<TextView>(Resource.Id.txtNoteIDANPR);
            _txtNoteMemo = view.FindViewById<TextView>(Resource.Id.txtNoteMemoANPR);
            _btnSave = view.FindViewById<Button>(Resource.Id.btnSaveANPR);
            _btnPhoto = view.FindViewById<Button>(Resource.Id.btnPhotoANPR);
           // _btnVideo = view.FindViewById<Button>(Resource.Id.btnVideo);
            _btnBack = view.FindViewById<Button>(Resource.Id.btnBackANPR);
           ...
           // _btnVideo.SetBackgroundDrawable(Resources.GetDrawable(Resource.Drawable.button_secondary));
            ...
            _imageView = view.FindViewById<ImageView>(Resource.Id.imageViewANPR);
	    ...
	    private async void BtnSaveClick(object sender, EventArgs e)
	    {
	        ...
	        var parkingSequenceADO = new ParkingSequenceADO();
	        //  if this note was added after the ticket was issued, we need to send the data immediately

	        bool submitNoteInfo = false;
	        //go get the parking tickets status
	        var commonADO = new CommonADO();
            var result = commonADO.GetRowInfoById(_parkNoteDTO.SeqId, structName);
        ...
        private void ShowNotesListFragment()
		{
            ...
            //var noteDetail = (NoteDetailFragment)FragmentManager.FindFragmentByTag(Constants.ANPR_DETAIL_FRAGMENT_TAG);
            //if (noteDetail != null)
        ...
        private void ShowANPRConfirmationFragment()
        {
            FragmentTransaction fragmentTransaction = FragmentManager.BeginTransaction();
            ...
            if (mIsLargeLayout)
            {
                ...
                ANPRDialogConfirm.Show(FragmentManager, Constants.ANPR_CONFIRMATION_FRAGMENT_TAG);
            }
            else
            {
